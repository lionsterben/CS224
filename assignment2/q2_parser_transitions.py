class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            raise Exception("can't match if else")
        ### END YOUR CODE
    ...
    while unfinished_parses:
        ...
        while(len(batch_parses) > 0):
            ...
            for ind, trans in enumerate(transitions):
                batch_parses[ind].parse_step(trans)
            batch_parses = [parse for parse in batch_parses if len(parse.stack) > 1 or len(parse.buffer) > 0]
