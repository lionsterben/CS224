...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        w = tf.get_variable(name="w", initializer=self.pretrained_embeddings)
        embedding_tmp = tf.nn.embedding_lookup(w, self.input_placeholder)
        embeddings = tf.reshape(embedding_tmp, [tf.shape(embedding_tmp)[0], -1]) 
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        W = tf.get_variable(name="W", initializer=xavier_weight_init()((self.config.n_features*self.config.embed_size, self.config.hidden_size)))
        b1 = tf.get_variable(name="b1", shape=(1, self.config.hidden_size), dtype=tf.float32, initializer=tf.constant_initializer(0.0))
        U = tf.get_variable(name="U", initializer=xavier_weight_init()((self.config.hidden_size, self.config.n_classes)))
        b2 = tf.get_variable(name="b2", shape=(1,self.config.n_classes), initializer=tf.constant_initializer(0.0))
