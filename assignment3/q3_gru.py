...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        # is True.
        # - Remember to set self.grad_norm
        gra_var = optimizer.compute_gradients(loss)
       # print(gra_var)
       # tf.Print(tf.shape(gra_var))
        gra = [gv[0] for gv in gra_var]
        var = [gv[1] for gv in gra_var]
        if self.config.clip_gradients:
            gra, _ = tf.clip_by_global_norm(gra, self.config.max_grad_norm)
            gra_var = zip(gra,var)
        self.grad_norm = tf.global_norm(gra)
       # print(tf.shape(gra_var))
        train_op = optimizer.apply_gradients(gra_var)
        ### END YOUR CODE
