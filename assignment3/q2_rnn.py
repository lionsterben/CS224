...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        more_len = max_length - length
        if more_len > 0:
            ret.append((sentence+[zero_vector]*more_len, labels+[zero_label]*more_len, mask+[0]*more_len))
        else:
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(dtype=tf.int32, shape=(None, self.max_length, self.config.n_features), name="input")
        self.labels_placeholder = tf.placeholder(dtype=tf.int32, shape=(None, self.max_length), name="label")
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        pretrained_embeddings = tf.get_variable(name="pre_embeddings", initializer = self.pretrained_embeddings)
        temp_embeddings = tf.nn.embedding_lookup(pretrained_embeddings, self.input_placeholder)
        embeddings = tf.reshape(temp_embeddings, [-1, self.max_length, self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable(name="U", shape=(self.config.hidden_size, self.config.n_classes), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
        b2 = tf.get_variable(name="b2", shape=(self.config.n_classes), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
        h = tf.zeros(shape=(tf.shape(self.input_placeholder)[0], cell.state_size),dtype=tf.float32)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                if time_step == 0:
                    output, hidden = cell(x[:,time_step,:], h)
                else:
                    output, hidden = cell(x[:,time_step,:],hidden)
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        temp = tf.boolean_mask(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds), self.mask_placeholder)
