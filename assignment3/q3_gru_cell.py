...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            b_r = tf.get_variable(name="b_r", shape=(self.state_size), initializer=tf.constant_initializer(value=0.0))
            ...
            b_z = tf.get_variable(name="b_z", shape=(self.state_size), initializer=tf.constant_initializer(value=0.0))
            ...
            b_o = tf.get_variable(name="b_o", shape=(self.state_size), initializer=tf.constant_initializer(value=0.0))
            z = tf.sigmoid(tf.matmul(inputs, W_z) + tf.matmul(state, U_z) + b_z)
            r = tf.sigmoid(tf.matmul(inputs, W_r) + tf.matmul(state, U_r) + b_r)
            o = tf.tanh(tf.matmul(inputs, W_o) + tf.matmul(tf.multiply(r, state), U_o) + b_o)
