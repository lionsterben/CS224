...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
		### YOUR CODE HERE (5-20 lines)
                ...
                for ind in range(sentence_length):
                    start_pad = []
                    end_pad = []
                    if ind < window_size:
                        start_pad = start[-word_length*(window_size-ind):]
                    if ind > sentence_length - 1 - window_size:
                        end_pad = end[:word_length*(window_size-(sentence_length-1-ind))]
                    middle = []
                    start_ind = max(0,ind-window_size)
                    end_ind = min(sentence_length-1, ind+window_size)
                    for i in range(start_ind, end_ind+1):
                        middle += sentence[i]                        
                    res = start_pad + middle + end_pad
                    windowed_data.append((res,labels[ind]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        pre_embedding = tf.get_variable(name="w",initializer=self.pretrained_embeddings)
        #pre_embedding = tf.constant(self.pretrained_embeddings)
        temp_embedding = tf.nn.embedding_lookup(pre_embedding, self.input_placeholder)
        embeddings = tf.reshape(temp_embedding, shape = [-1, self.config.n_window_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name="W", shape=(self.config.n_window_features*self.config.embed_size, self.config.hidden_size), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        b = tf.get_variable(name="b", shape=(1, self.config.hidden_size), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        ...
        b2 = tf.get_variable(name="b2", shape=(1, self.config.n_classes), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
